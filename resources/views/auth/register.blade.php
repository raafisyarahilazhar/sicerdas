...
<body class="bg-white h-screen overflow-hidden">
    ...
    <main class="flex flex-col md:flex-row h-screen">
        ...
        <!-- Kolom Hijau (Hilang di mobile) -->
        <div class="hidden md:flex w-full md:w-2/5 bg-green-600 text-white p-12 flex-col justify-center items-center text-center space-y-6 bg-cover bg-center h-screen"
            ...
            <a href="{{ route('login') }}"
                class="inline-block border-2 border-white rounded-full px-8 py-2 md:px-12 md:py-3 font-semibold uppercase hover:bg-white hover:text-green-600 transition-colors duration-300">
                Sign In
            </a>
        </div>

  <!-- Kolom Putih (Form) dengan scroll sendiri -->
        <div class="w-full md:w-3/5 bg-white p-8 sm:p-12 flex flex-col items-center overflow-y-auto">
            <div class="w-full max-w-md space-y-6">
                ...
                <form action="{{ route('register') }}" method="POST" class="space-y-6">
                    ...
                    <div class="relative">
                        <span class="input-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 4a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 4a1 1 0 100 2h4a1 1 0 100-2H7z" clip-rule="evenodd" />
                            </svg>
                        </span>
                        <input type="text" name="nik" placeholder="NIK" maxlength="16" minlength="16" class="input-field" required>
                        @error('nik')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
  ...
  @error('rt_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
    ...
    <script>
        (function () {
            ...
            // reset pilihan
            rtSelect.value = '';
            // aktifkan select RT setelah RW dipilih
            rtSelect.disabled = !rwId;

            // tampilkan hanya RT yang punya data-rw sama dengan rwId
            Array.from(rtSelect.options).forEach(opt => {
                // biarkan placeholder tetap terlihat
                if (!opt.value) { opt.hidden = false; return; }
                opt.hidden = (opt.getAttribute('data-rw') !== rwId);
            });
            }
            ...
            // jika ada nilai lama (old) dari server, pilihkan kembali
            const oldRw = @json(old('rw_id'));
            const oldRt = @json(old('rt_id'));
            if (oldRw) {
            rwSelect.value = String(oldRw);
            filterRT();
            if (oldRt) rtSelect.value = String(oldRt);
            }

            // saat RW berubah → filter RT
