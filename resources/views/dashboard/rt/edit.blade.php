...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 lg:p-10">
    <div class="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg">
        ...
        <form method="POST" action="{{ route('rts.update', $rt->id) }}" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
                <input type="text" name="name" id="name" value="{{ old('name', $rt->name ) }}" class="input-style" placeholder="Masukkan nama RW" required>
                @error('name') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
            </div>

            <div>
                <label for="nomor_rt" class="block text-sm font-medium text-gray-700 mb-1">Nomor RT</label>
                <input type="number" name="nomor_rt" id="nomor_rt" value="{{ old('nomor_rw', $rt->nomor_rt ) }}" class="input-style" placeholder="Masukkan nomor RT" required>
